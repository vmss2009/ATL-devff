...
class FFChatInfo {
  ...
  final List<AtlUsersRecord>? groupMembers;

  AtlUsersRecord get currentUser => groupMembers!
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, AtlUsersRecord> get otherUsers => Map.fromEntries(
        groupMembers!
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<AtlUsersRecord> get otherUsersList => otherUsers.values.toList();
...
class FFChatManager {
  ...
  Stream<FFChatInfo> getChatInfo({
    AtlUsersRecord? otherUserRecord,
    DocumentReference? chatReference,
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => AtlUsersRecord.getDocument(ref).first),
      );
